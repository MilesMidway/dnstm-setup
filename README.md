...
Interactive DNS Tunnel Setup Wizard for censorship-resistant internet access.

Automates the complete deployment of [dnstm](https://github.com/net2share/dnstm) DNS tunnel servers with **Slipstream** and **DNSTT** protocols. Designed to help people in restricted regions stay connected to the free internet.
...
DNS (Domain Name System) is the internet's phone book — every device on the planet needs it to work. DNS tunneling exploits this by encoding your internet traffic inside DNS queries and responses.
...
2. These queries look like normal DNS lookups (e.g., "what is abc123.t2.yourdomain.com?")
3. The queries travel through public DNS resolvers (Google 8.8.8.8, Cloudflare 1.1.1.1, etc.)
4. Your server receives the queries, decodes the hidden data, and forwards it to the real internet
5. Responses travel back the same way, encoded inside DNS responses
...
```
  ...
  DNS Router (multiplexes port 53)
    |
    +---> t2.domain ---> Slipstream Server ---> microsocks (:19801) ---> Internet
    |                    (QUIC + TLS)
    |
    +---> d2.domain ---> DNSTT Server --------> microsocks (:19801) ---> Internet
    |                    (Noise + Curve25519)
    |
    +---> s2.domain ---> Slipstream Server ---> SSH Tunnel ------------> Internet
...
|---|---|---|
| **dnstm** | DNS Tunnel Manager | CLI tool that manages all tunnel binaries, services, and routing |
| **DNS Router** | Port 53 multiplexer | Inspects incoming DNS queries and routes them to the correct tunnel based on subdomain |
| **Slipstream Server** | QUIC-based DNS tunnel | Uses TLS encryption with self-signed certificates. Speed: ~63 KB/s |
| **DNSTT Server** | Classic DNS tunnel | Uses Noise protocol with Curve25519 key pairs. Speed: ~42 KB/s |
| **microsocks** | SOCKS5 proxy | Lightweight proxy running on port 19801, shared by all tunnels |
| **sshtun-user** | SSH user manager | (Optional) Creates restricted users that can only do port forwarding — no shell access |

### Three Tunnel Types

| Tunnel | Subdomain | Transport | Backend | Use Case |
|---|---|---|---|---|
| **slip1** | `t2.domain` | Slipstream (QUIC) | SOCKS | Fastest option, recommended for most users |
...
**SSH backend:** Requires username + password. Provides per-user access control. The SSH user is restricted — even if credentials leak, no one can access your server (only tunnel traffic).
...
The script guides you through everything interactively. Press **h** at any prompt for detailed help.
...
- Detects `systemd-resolved` (Ubuntu's local DNS cache) by both process name and `127.0.0.53` address
...
### Step 10 — SSH Tunnel User (Optional)
...
This tells the internet: "ns.yourdomain.com is at this IP address."
...
These tell the internet: "For any queries about t2/d2/s2.yourdomain.com, ask ns.yourdomain.com (your server)."

### Common Mistakes
- **Using `tns` instead of `ns`** for the A record name — it must be exactly `ns`
- **Leaving Cloudflare proxy ON** (orange cloud) — this breaks DNS tunneling because queries go to Cloudflare instead of your server
- **Setting NS record values to the IP** — NS records must point to a hostname (`ns.yourdomain.com`), not an IP address
...
# Show project information (no root needed)
...
Each topic gives deep explanations of how things work, why each step is needed, and what the terminology means. You can browse multiple topics and return to the setup prompt when ready.
...
| **DNSTT Public Key** | The key shown in Step 7 (only for d2 DNSTT tunnel) |

### Recommended DNS Resolvers

Use any of these in SlipNet's resolver setting:

| Resolver | IP | Provider |
|---|---|---|
| Google | `8.8.8.8:53` | Google Public DNS |
| Cloudflare | `1.1.1.1:53` | Cloudflare DNS |
| Quad9 | `9.9.9.9:53` | Quad9 |
| OpenDNS | `208.67.222.222:53` | Cisco OpenDNS |
| AdGuard | `94.140.14.14:53` | AdGuard DNS |
| CleanBrowsing | `185.228.168.9:53` | CleanBrowsing |
...
- DNS records in Cloudflare — delete them from your dashboard
- systemd-resolved — re-enable with:
  ```bash
...
**[Complete Guide to Setting Up a DNS Tunnel](https://telegra.ph/Complete-Guide-to-Setting-Up-a-DNS-Tunnel-03-04)**
...
4. سرور شما درخواست‌ها را دریافت، رمزگشایی و به اینترنت واقعی ارسال می‌کند
5. پاسخ‌ها به همین روش برمی‌گردند
...
اسکریپت به صورت تعاملی شما را در تمام مراحل راهنمایی می‌کند. در هر مرحله کلید **h** را بزنید تا راهنمای کامل آن بخش نمایش داده شود.
...
- **روشن بودن پروکسی Cloudflare** (ابر نارنجی) — باید خاموش باشد (ابر خاکستری)
- **قرار دادن IP به جای دامنه** در مقدار NS — مقدار NS باید `ns.yourdomain.com` باشد نه آدرس IP
...
8. **شروع سرویس‌ها** — راه‌اندازی روتر و تمام تانل‌ها
...
- رکوردهای DNS در Cloudflare — از داشبورد حذف کنید
- systemd-resolved — برای فعال‌سازی مجدد:
